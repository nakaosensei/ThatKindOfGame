class Menu extends GameState {
  ...
  create() {
    ...
    this.game.time.events.add(Phaser.Timer.SECOND * 6, this.fadePictureAdrian,
      this);
  ...
  showLogo() {
    this.logoSprite = this.game.add.sprite((this.game.width - this.game.width /
      2) - this.game.width * 0.05, (this.game.height - this.game.height /
      2) - 60, 'logo');
    this.logoSprite.anchor.setTo(0.5, 0.5);
    this.logoSprite.alpha = 0;
    this.game.add.tween(this.logoSprite).to({
      alpha: 1
    ...
    let bbIcon = this.game.add.sprite(this.game.width - 10, this.game.height -
      10, 'begin')
  ...
  fadePictureAdrian() {
    this.game.add.tween(this.adrian).to({
      alpha: 0
